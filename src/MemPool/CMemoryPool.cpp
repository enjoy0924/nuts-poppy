...
{
    ACE_Guard<ACE_Mutex> guard(m_syncMutex);
    //lock here when pool allocate memory, there's no lock when free allocated memory
...
void CMemoryPool::FreeMemory(void *ptrMemoryBlock, const std::size_t &sMemoryBlockSize)
{
  ...
  // so it becomes available to the Memory-Pool again...
